...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const playerBonusContextId = req.body;
  try {
    const response = await client.post('/bonus/decline', playerBonusContextId);
    res.status(response.status).json({ message: 'Bonus Cancelled!' });
  } catch (error) {
